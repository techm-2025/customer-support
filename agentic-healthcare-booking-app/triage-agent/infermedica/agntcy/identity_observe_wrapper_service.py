...
from observe import initialize_observability
...
class A2ATriageService:
    ...
    @agent(name="handle_a2a_message", description="process incoming a2a message from healthcare agent", version=1)
    ...
    @agent(name="create_new_triage_task", description="create new triage task and start triage session", version=1)
    ...
    @agent(name="continue_existing_triage_task", description="continue existing triage task with user input", version=1)
...
def main():
    ...
    try:
        ...
        Observe.init("a2a_triage_service",api_endpoint=api_endpoint)
